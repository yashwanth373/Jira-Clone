...
export class ProjectboardComponent implements OnInit {
  constructor(
    ...
    private _dsService: DataService,
      ...
      if (this.groupByOption === 'None') {
        ...
        for (let section of this.project.board) {
          this.activeSprint[section].sort((a: any, b: any) => {
            if (parseInt(a.modifiedAt) > parseInt(b.modifiedAt)) return -1;
            else if (parseInt(a.modifiedAt) < parseInt(b.modifiedAt)) return 1;
            else return 0;
          });
        }
        ...
        for (let section of this.project.board) {
          ...
          for (let user of users) {
            let userIssues = this.activeSprint[section].filter(
              (issue: any) => issue.assignee.user_id === user
            );
        ...
        for (let section of this.project.board) {
          ...
          for (let priority of priorities) {
            let priorityIssues = this.activeSprint[section].filter(
              (issue: any) => issue.priority === priority
            );
    ...
    for (let section of this.project.board) {
      for (let issue of this.activeSprint[section]) {
        if (section !== issue.status) {
          ...
          this._dsService
            .updateIssue(this.project.project_id, issue)
            .subscribe((data: any) => {
  ...
  deleteSectionfromStageArea(index: any) {
    ...
    let toSection =
      index === 0 ? this.stagingArea[1] : this.stagingArea[index - 1];
  ...
  updateBoard() {
    ...
    this._dsService
      .updateBoard(this.project.project_id, this.project.board)
      .subscribe((data: any) => {
  ...
  HRDateFormat(timestamp: any, isDueDate: any) {
    ...
    if (isDueDate) return month + ' ' + day2 + ', ' + year;
    return (
      '' +
      month +
      ' ' +
  ...
  deleteIssue() {
    ...
    this.activeSprint.issues = this.activeSprint.issues.filter(
      (id: any) => id !== this.selectedIssue.issue_id
    );
  ...
  saveEditedIssue() {
    ...
    if (
      ...
      let foundIndex = this.activeSprint[this.selectedIssue.status].findIndex(
        (issue: any) => issue.issue_id === this.selectedIssue.issue_id
      );
      let foundIndex2 = this.project.Issues.findIndex(
        (issue: any) => issue.issue_id === this.selectedIssue.issue_id
      );

      this.activeSprint[this.selectedIssue.status].splice(foundIndex, 1, {
        ...this.selectedIssue,
      });
  ...
  async onChange(files: any) {
    ...
    this.selectedIssue.attachment = this.selectedIssue.attachment.concat(attachments);
    console.log(this.selectedIssue);
